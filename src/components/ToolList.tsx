  ...
  return (
    <div className="tool-meta-card">
      <div className="tool-meta-header">
        ...
        <span className={`tool-meta-permission ${tool.permission}`}>
          {permissionLabels[tool.permission]}
        </span>
      </div>
      ...
      <div className="tool-meta-footer">
        ...
        {tool.requirements && (
          <span className="tool-meta-requirements">{tool.requirements}</span>
        )}
      </div>
    </div>
  )
...
export function ToolList() {
  ...
  for (const tool of tools) {
    const group = groups.get(tool.source) ?? []
    group.push(tool)
  ...
  return (
    <div className="tools-panel">
      {[...groups.entries()].map(([source, groupTools]) => (
        <section key={source} className="tools-group">
          <h2 className="tools-group-title">
            <Wrench size={16} />
            {sourceLabels[source]}
            <span className="tools-group-count">{groupTools.length}</span>
          </h2>
